 ...
 */
- (NSArray *)parsFileCodeVertical:(char)verticalRow
                          top:(NSInteger)top
                        ...
                        limit:(BOOL)isLimit
{
    ...
    for (NSInteger i = row_top; i <= row_bottom; i ++) {
        NSString *content = [[_firstWorksheet cellForCellReference:[NSString stringWithFormat:@"%@%lu", flag, i]] stringValue];
        NSString *nextContent = [[_firstWorksheet cellForCellReference:[NSString stringWithFormat:@"%@%lu", flag, i + 1]] stringValue];
        
        // 如果当前空则跳过找下一行
        if (content.length == 0 && nextContent.length != 0) {
            continue;
        }
        
        // 如果下一行也为空，则结束
        if (!isLimit && content.length == 0 && nextContent.length == 0) {
            break;
        }
